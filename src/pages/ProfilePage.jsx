  ...
  return (
    <div style={{ background:'var(--bg)' }} className="min-h-screen">
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        {/* Profile Header */}
        <div className="bg-white rounded-card border border-blue-brand/10 overflow-hidden mb-6">
          <div className="h-28 bg-gradient-to-r from-blue-brand via-blue-mid to-blue-light relative">
          ...
          <div className="px-6 pb-5">
            <div className="flex items-end justify-between flex-wrap gap-3" style={{marginTop:'-2.5rem'}}>
              <div className="flex items-end gap-4 flex-wrap">
                <div className="w-20 h-20 rounded-full bg-blue-brand border-4 border-white text-white font-display font-black text-2xl flex items-center justify-center shadow-lg flex-shrink-0">
                  {initials}
                </div>
                <div className="pb-2">
          ...
          <div className="bg-white rounded-card border border-blue-brand/10 p-6">
            ...
            {bkLoading ? (
              ...
              <div className="flex flex-col gap-3">
                {bookings.map(b => (
                  <div key={b.id} className="flex items-center gap-3 p-4 bg-gray-50 rounded-[9px] border border-blue-brand/8 hover:border-blue-brand/20 transition-colors">
        ...
        {tab === 'preferences' && (
          <div className="bg-white rounded-card border border-blue-brand/10 p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => navigate('/auth')} className="text-sm font-semibold text-blue-brand hover:underline">✎ Retake Quiz</button>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {[['Interests','No interests set yet'],['Budget','No budget set'],['Travel Style','Not set'],['Travel Group','Not set']].map(([label, fallback]) => (
                <div key={label} className="p-4 bg-blue-tint rounded-[9px] border border-blue-brand/10">
            ...
            <div className="mt-4 p-4 bg-blue-tint rounded-[9px] border border-blue-brand/15">
              <div className="text-xs font-bold text-blue-brand mb-1">RECOMMENDATION ACCURACY</div>
              <div className="text-xs text-gray-500">Complete your quiz to get personalised results tailored to your interests and budget.</div>
              <button onClick={() => navigate('/auth')} className="btn-primary text-xs px-4 py-2 mt-3">Complete Quiz →</button>
            </div>
          </div>
        )}
